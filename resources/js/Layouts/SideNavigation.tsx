            ...
            <ul className="relative px-1">
                {accounts.map((account:any) => {
                    return <li key={account.id} className="relative">
                        <Link className="flex items-center text-sm py-4 px-6 h-12 overflow-hidden text-gray-700 text-ellipsis whitespace-nowrap rounded hover:text-blue-600 hover:bg-blue-50 transition duration-300 ease-in-out" href={`/account/${account.id}/show`} data-mdb-ripple="true" data-mdb-ripple-color="primary">
                           ...
                           {/* <Link href={`/account/${account.id}/show`}>{account.account_name}</Link>*/}
                        </Link>
                    </li>
                })}
            </ul>
